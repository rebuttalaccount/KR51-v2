...
def main(
    ...
    for batch_idx in tqdm(range(num_batches)):
        ...
        for in_batch_idx in range(max_batch_size):
            ...
            # texture
                ...
                texture = dtd_classes[(batch_idx * max_batch_size + in_batch_idx) % 47]
        ...
        for texture_, result in zip(keys, results):
            generation = result['generation']
            new_prompt = generation.split('\n')[0]
            new_prompt = new_prompt.replace('\n', ' ')
            new_prompt = new_prompt.strip()
            new_prompts.append([texture_, new_prompt])

    generate_dict = {}
    for message in new_prompts:
        if message[0] not in generate_dict:
            generate_dict[message[0]] = [message[1]]
        else:
            generate_dict[message[0]].append(message[1])
    with open(new_prompt_filename, "w", encoding="utf-8") as f:
        json.dump(generate_dict, f, ensure_ascii=False, indent=4)
    # min = 1000000
    ...
    #         min = len(v)
    # print(min)
    # r = open(new_prompt_filename, "wb")
    # pickle.dump(generate_dict, r)
    # r.close()
